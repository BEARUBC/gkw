...
xin.setStreamName('in') #host - device communication 

script = pipeline.create(dai.node.Script)
xin.out.link(script.inputs['in']) #Device to host, multiple inputs???
script.setScript("""
    import json
...
xout = pipeline.create(dai.node.XLinkOut)#Okd --> device 
...
with dai.Device(pipeline) as device:
    ...
    buffer = dai.Buffer() 
    buffer.setData(list(data)) # Buffering communication from host to device 
    device.getInputQueue("in").send(buffer) # From "in" link, send data to buffer

    # Wait for the script to send the changed dictionary back
    jsonData = device.getOutputQueue("out").get() # Once available recieve data 
