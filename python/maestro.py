...
def setRangeFn(chan, min, max):
    controller.setRange(chan, min, max)


def getMinFn(chan):
    controller.getMin(chan)


def getMaxFn(chan):
    controller.getMax(chan)


def setTargetFn(chan, target):
    controller.setTarget(chan, target)


def setSpeedFn(chan, speed):
    controller.setSpeed(chan, speed)


def setAccelFn(chan, accel):
    controller.setAccel(chan, accel)


def getPositionFn(chan):
    controller.getPosition(chan)


def isMovingFn(chan):
    controller.isMoving(chan)


def getMovingStateFn():
    controller.getMovingState()


def runScriptSub(subNumber):
    controller.runScriptSub(subNumber)
